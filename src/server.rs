...
mod tests {
    ...
    async fn full_hot_potato_roundtrip_over_http() {
        ...
        // read receipt
        let res = rpc_call(app(bus.clone()), "message/read", json!({"agent":"the quant agent","id":id})).await;
        assert_eq!(res["result"]["status"], "read");

        // ack
        let res = rpc_call(app(bus.clone()), "message/ack", json!({"agent":"the quant agent","id":id,"note":"done"})).await;
        assert_eq!(res["result"]["status"], "acked");

        // status shows the lifecycle to the sender
        let res = rpc_call(app(bus.clone()), "agent/status", json!({"agent":"the pm agent"})).await;

...
mod tests {
    ...
    async fn send_poll_reply_ack_roundtrip() {
        let b = bus().await;
        let id = b
            .send("the pm agent", "the quant agent", MsgType::Task, "run X", "body", None)
            .await
            .unwrap();

        let mail = b.poll("the quant agent", 0).await.unwrap();
        ...
        let acked = b.ack("the quant agent", &id, "delivered csv").await.unwrap();
        assert_eq!(acked.status, crate::message::MessageStatus::Acked);

        let p_status = b.status("the pm agent").await.unwrap();
        assert_eq!(p_status.len(), 1); // only the task she sent (reply is the quant agent's)
    }
    ...
    async fn worker_cannot_broadcast() {
        let b = bus().await;
        let err = b
            .send("the quant agent", "*", MsgType::Broadcast, "spam", "body", None)
            .await
            .unwrap_err();
    ...
    async fn reply_without_ref_rejected() {
        let b = bus().await;
        let err = b
            .send("the quant agent", "the pm agent", MsgType::Reply, "re:", "x", None)
            .await
            .unwrap_err();
    ...
    async fn unregistered_sender_rejected() {
        let b = bus().await;
        let err = b
            .send("mallory", "the quant agent", MsgType::Task, "hi", "b", None)
            .await
            .unwrap_err();
    ...
    async fn chatlog_timestamps_flow() {
        let b = bus().await;
        let id = b
            .send("the pm agent", "the quant agent", MsgType::Task, "t", "b", None)
            .await
            .unwrap();

        // queued: no read receipt yet
        let s0 = b.status("the pm agent").await.unwrap();
        assert!(s0[0].read_at.is_none());

        b.poll("the quant agent", 0).await.unwrap();
        let read = b.mark_read("the quant agent", &id).await.unwrap();
        ...
        let acked = b.ack("the quant agent", &id, "done").await.unwrap();
    ...
    async fn read_before_delivery_rejected() {
        ...
        let id = b
            ...
            .await
            .unwrap();
        let err = b.mark_read("the quant agent", &id).await.unwrap_err();
    ...
    async fn archive_only_counts_acked() {
        ...
        b.poll("the quant agent", 0).await.unwrap();
        b.ack("the quant agent", &id, "done").await.unwrap();
    ...
    async fn delete_one_removes_exactly_that_letter() {
        let b = bus().await;
        let keep = b.send("the pm agent", "the quant agent", MsgType::Task, "keep", "b", None).await.unwrap();
        let kill = b.send("the pm agent", "the quant agent", MsgType::Task, "kill", "b", None).await.unwrap();
    ...
    async fn delete_all_clears_every_status() {
        let b = bus().await;
        b.send("the pm agent", "the quant agent", MsgType::Task, "queued", "b", None).await.unwrap();
        let did = b.send("the pm agent", "the quant agent", MsgType::Task, "delivered", "b", None).await.unwrap();
        b.poll("the quant agent", 0).await.unwrap();
        b.ack("the quant agent", &did, "done").await.unwrap();

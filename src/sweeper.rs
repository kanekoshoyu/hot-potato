//! Queue sweeper — the self-healing dispatcher (Patricia, 2026-09-16).
//!
...
mod tests {
    ...
    async fn bus_with_registry() -> (Arc<EventBus>, Arc<Registry>) {
        ...
        bus.register("patricia", crate::bus::Role::Pm).await.unwrap();
        bus.register("victoria", crate::bus::Role::Worker).await.unwrap();
    ...
    async fn sweep_recovers_letter_push_failed_at_send_time() {
        ...
        bus.send("patricia", "victoria", MsgType::Task, "subject", "body", None)
            .await
            .unwrap();
    ...
    async fn sweep_respects_backoff_and_never_touches_nonqueued() {
        ...
        bus.send("patricia", "victoria", MsgType::Task, "fresh", "body", None)
            .await
            .unwrap();
        ...
        bus.mark_delivered("victoria", &queued_id).await.unwrap();
        let n = sweep_once(&bus, &registry, &transport, &hub, Duration::ZERO).await;
    ...
    async fn sweep_skips_unregistered_receiver_without_touching_mailbox() {
        ...
        bus.send("patricia", "sho", MsgType::Task, "to a poll-only human", "body", None)
            .await
            .unwrap();

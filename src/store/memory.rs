...
mod tests {
    ...
    async fn setup() -> InMemoryStore {
        ...
        s.register("the pm agent").await.unwrap();
        s.register("the quant agent").await.unwrap();
        s
    }
    ...
    async fn full_lifecycle() {
        ...
        // peek doesn't mark
        assert_eq!(s.peek("the quant agent").await.unwrap().len(), 1);
        assert_eq!(s.peek("the quant agent").await.unwrap().len(), 1);

        // poll marks delivered
        let drained = s.poll("the quant agent", 10).await.unwrap();
        assert_eq!(drained.len(), 1);
        assert_eq!(drained[0].status, MessageStatus::Delivered);
        assert!(s.peek("the quant agent").await.unwrap().is_empty());
        ...
        assert_eq!(s.sent_by("the pm agent").await.unwrap().len(), 1);
    }
    ...
    async fn fifo_order_preserved() {
        let s = setup().await;
        for i in 0..5 {
            s.push(&Message::new("the pm agent", "the quant agent", MsgType::Task, format!("m{i}"), "b", None))
                .await
                .unwrap();
        }
        let drained = s.poll("the quant agent", 10).await.unwrap();
    ...
    async fn mailbox_full_is_an_error() {
        ...
        s.register("the quant agent").await.unwrap();
        for _ in 0..2 {
            s.push(&Message::new("p", "the quant agent", MsgType::Task, "s", "b", None))
                .await
                .unwrap();
        }
        let err = s
            .push(&Message::new("p", "the quant agent", MsgType::Task, "s3", "b", None))
            .await
            .unwrap_err();

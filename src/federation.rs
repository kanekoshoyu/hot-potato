...
//!       "agents":["the pm agent","the quant agent","the data agent","the research agent"]}]
...
mod tests {
    ...
    fn resolves_remote_agents_only() {
        let p = Peers::from_json(CFG).unwrap();
        assert!(p.claims("the pm agent"));
        assert_eq!(p.resolve("the pm agent").unwrap().name(), "fleet");
        assert!(!p.claims("the infra agent"));
        assert!(p.resolve("the infra agent").is_none());
    }

    #[tokio::test]
    async fn forward_refuses_hops_over_limit() {
        let p = Peers::from_json(CFG).unwrap().resolve("the quant agent").unwrap();
        let mut letter = Message::new("the infra agent", "the quant agent", crate::message::MsgType::Task, "s", "b", None);

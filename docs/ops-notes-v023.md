...
- albatross: the quant agent 建的 version-bump CD（改 apps/*/Cargo.toml version → Coolify webhook）
...
- daometric.com 网站：astro build → rsync nginx；runner `daometric-host` + nginx
  都在 the infra agent 机（169.58.177.121）——不在本机！
...
- the infra agent 机 = prod potato + trader + Coolify + daometric.com nginx/runner
- Coolify API key 只在 the quant agent 机的 apps/research/.env（本机没有）
- trader 0.88.3 卡点 = the infra agent 机 Coolify 构建（night watch: /tmp/night-watch.log）
